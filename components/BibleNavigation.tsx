...
const BibleNavigation: React.FC<BibleNavigationProps> = ({ language, onSelectPassage, isOpen, onClose }) => {
  ...
  const getBookName = (book: BibleBook, lang: Language): string => {
    switch (lang) {
      case 'zh': return book.zh;
      case 'zh-CN': return book.zhCN;
      case 'ja': return book.ja;
      case 'hi': return book.hi;
      case 'es': return book.es;
      case 'fr': return book.fr;
      case 'pt': return book.pt;
      case 'it': return book.it;
      case 'de': return book.de;
      case 'ko': return book.ko;
      default: return book.en;
    }
  };
      ...
      >
        ...
        <div className="p-4 border-b border-bible-100 flex items-center justify-between bg-bible-50">
          <div className="flex items-center gap-2 text-bible-900 font-display font-bold">
            ...
            <span>{(language === 'zh' || language === 'zh-CN') ? '聖經目錄' : 'Bible Index'}</span>
          </div>
          <button 
            onClick={onClose}
