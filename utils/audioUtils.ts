...
export const splitTextIntoChunks = (text: string): string[] => {
  ...
  // Split by common punctuation but CAPTURE the delimiter so we don't lose it.
  // The capture group () in split causes the delimiter to be included in the array.
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (i % 2 === 0) {
      ...
      currentSentence += parts[i]; // Append delimiter
  ...
  // If there's any leftover text without punctuation, add it
  if (currentSentence.trim()) {
    sentences.push(currentSentence.trim());
  }
  ...
  for (const sentence of sentences) {
    ...
    if ((currentChunk + sentence).length > TARGET_CHUNK_LENGTH) {
      ...
      // Add space if needed when combining
